...
export const taskManager = {
    ...
    colorSatatus(status){
        switch(status){
            case "em andamento": {return chalk.bgHex("#b27f00") (` ${status} `);}
            case "concluída": {return chalk.bgGreen(` ${status} `);}
            case "cancelada": {return chalk.bgRed(` ${status} `);}
            default: {return chalk.bgWhite(` ${status} `);}
